...
const REJECTION_GUIDANCE: Record<string, { title: string; guidance: string }> = {
  ...
  IG005: { title: 'This workbook is not safe to open', guidance: 'Export a fresh XLSX or CSV copy from the source system.' },
  IG006: { title: 'This file contains too much data', guidance: 'Split it into smaller files and try again.' },
  IG007: { title: 'Required columns were not found', guidance: 'Check the column headings and upload a corrected file.' },
  IG008: { title: 'The amount column is unclear', guidance: 'Use one clearly labelled amount column.' },
  IG009: { title: 'A value could not be read safely', guidance: 'Check the highlighted source row and its number or date formatting.' },
  IG010: { title: 'Formulas are not accepted', guidance: 'Replace formulas with their final values before uploading.' },
  IG011: { title: 'The workbook has multiple possible sheets', guidance: 'Keep one relevant visible sheet and try again.' },
...
export function humanizeIngestReject(code: string): { title: string; guidance: string } {
  return REJECTION_GUIDANCE[code] ?? {
