...
async function listTasks(): Promise<unknown> {
  return [
    { task_id: 'receivables-eu', task_type: 'allocation_upsert', description: 'Receivables reconciliation — correct remittance allocations' },
    { task_id: 'vendor-bank-eu', task_type: 'vendor_bank_update', description: 'Vendor bank-detail change governance (Treasury master-data)' },
  ];
}
...
async function stageAllocationCorrection(
  ...
  const r = await d.query(
    `SELECT ${SCHEMA}.stage_change($1,$2,$3,$4::jsonb) AS proposal_id`,
    ['receivables-eu', 'allocation_upsert', 'cfg-recv-v1', JSON.stringify(diff)],
  );
...
async function stageVendorBankUpdate(
  ...
  const r = await d.query(
    `SELECT ${SCHEMA}.stage_change($1,$2,$3,$4::jsonb) AS proposal_id`,
    ['vendor-bank-eu', 'vendor_bank_update', 'cfg-vend-v1', JSON.stringify(diff)],
  );
  return { proposal_id: r.rows[0]?.['proposal_id'], staged: diff };
}

async function listProposals(d: UserDb): Promise<Record<string, unknown>[]> {
  const r = await d.query(
    `SELECT proposal_id, task_id, change_type, state, proposer_id, approver_id, diff
...
const TOOLS = [
  { type: 'function', function: { name: 'list_tasks', description: 'List the automation types this co-worker can run.', parameters: { type: 'object', properties: {} } } },
  { type: 'function', function: { name: 'list_remittances', description: 'List demo receivables remittances with their current allocated sum vs total.', parameters: { type: 'object', properties: {} } } },
  { type: 'function', function: { name: 'list_vendors', description: 'List vendors and their current bank IBAN on file.', parameters: { type: 'object', properties: {} } } },
  { type: 'function', function: { name: 'get_proposal', description: 'Fetch one proposal by id.', parameters: { type: 'object', properties: { proposal_id: { type: 'string' } }, required: ['proposal_id'] } } },
  { type: 'function', function: { name: 'stage_allocation_correction', description: 'Stage (NOT commit) a correction to a remittance allocation amount. A different person must approve.', parameters: { type: 'object', properties: { remittance_id: { type: 'string' }, allocation_id: { type: 'string' }, new_amount: { type: 'number' } }, required: ['remittance_id', 'allocation_id', 'new_amount'] } } },
  { type: 'function', function: { name: 'stage_vendor_bank_update', description: 'Stage (NOT commit) a vendor bank-detail (IBAN/BIC) change. A different person must approve.', parameters: { type: 'object', properties: { vendor_id: { type: 'string' }, new_iban: { type: 'string' }, new_bic: { type: 'string' }, effective_date: { type: 'string' } }, required: ['vendor_id', 'new_iban', 'new_bic', 'effective_date'] } } },
];
...
async function runTool(d: UserDb, name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'list_remittances': return listRemittances(d);
    case 'list_vendors': return listVendors(d);
    case 'get_proposal': return getProposal(d, String(args['proposal_id']));
    case 'stage_allocation_correction':
      return stageAllocationCorrection(d, { remittance_id: String(args['remittance_id']), allocation_id: String(args['allocation_id']), new_amount: Number(args['new_amount']) });
    case 'stage_vendor_bank_update':
      return stageVendorBankUpdate(d, { vendor_id: String(args['vendor_id']), new_iban: String(args['new_iban']), new_bic: String(args['new_bic']), effective_date: String(args['effective_date']) });
...
export function setupReconRoutes(appkit: AppKitOBO): void {
  appkit.server.extend((app) => {
    app.get('/api/proposals', async (req: Request, res: Response) => {
      try {
        res.json({ identity: actorOf(req), proposals: await listProposals(db(appkit, req)) });
      } catch (err) { res.status(500).json({ error: (err as Error).message }); }
    });

    app.post('/api/chat', async (req: Request, res: Response) => {
      const d = db(appkit, req);
      const userMsg = String((req.body as { message?: unknown })?.message ?? '');
      ...
      try {
        for (let i = 0; i < 5; i++) {
          ...
          if (toolCalls.length === 0) {
            res.json({ identity: actorOf(req), reply: String(msg['content'] ?? ''), tool_events: events, proposals: await listProposals(d) });
            return;
          }
          for (const tc of toolCalls) {
            let parsed: Record<string, unknown> = {};
            try { parsed = JSON.parse(tc.function.arguments || '{}') as Record<string, unknown>; } catch { parsed = {}; }
            let result: unknown;
            try { result = await runTool(d, tc.function.name, parsed); }
            catch (e) {
        ...
        res.json({ identity: actorOf(req), reply: '(stopped after 5 steps)', tool_events: events, proposals: await listProposals(d) });
      } catch (err) {
        res.status(500).json({ identity: actorOf(req), error: (err as Error).message, tool_events: events });
      }
    });

    app.post('/api/approve', async (req: Request, res: Response) => {
      const id = String((req.body as { proposal_id?: unknown })?.proposal_id ?? '');
      try {
        const r = await db(appkit, req).query(`SELECT ${SCHEMA}.approve_change($1) AS result`, [id]);
    ...
    app.post('/api/commit', async (req: Request, res: Response) => {
      ...
      try {
        const r = await db(appkit, req).query(`SELECT ${SCHEMA}.commit_change($1,$2,'user') AS result`, [id, actor]);
        const result = r.rows[0]?.['result'];
        const audit = await db(appkit, req).query(
          `SELECT actor_id, db_principal, payload_sha256 FROM ${SCHEMA}.audit_event WHERE proposal_id = $1`, [id],
        );
